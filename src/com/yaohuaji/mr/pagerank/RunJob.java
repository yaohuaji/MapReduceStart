package com.yaohuaji.mr.pagerank;
...
public class RunJob {
	...
	public static void main(String[] args) {
		...
		while(true){
			...
			try {
				...
				Path inputPath =new Path("/root/input/pagerank/pagerank.txt");
				if(i>1){
	...
	static class PageRankReducer extends Reducer<Text, Text, Text, Text>{
		protected void reduce(Text arg0, Iterable<Text> arg1,
			...
			int j=(int)( d*1000.0);//increment需要一个long型
			j=Math.abs(j);
			System.out.println(j+"___________");
			arg2.getCounter(Mycounter.my).increment(j);//Mapreduce独有的计数器
