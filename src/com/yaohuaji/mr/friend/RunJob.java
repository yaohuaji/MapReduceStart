package com.yaohuaji.mr.friend;
...
public class RunJob {
	...
	public static void run2(Configuration config) {
		try {
			...
			//设置MR执行的输入文件
			FileInputFormat.addInputPath(job, new Path("/root/output/f1"));
			
			//该目录表示MR执行之后的结果数据所在目录，必须不能存在
			Path outputPath=new Path("/root/output/f2");
	...
	public static boolean run1(Configuration config) {
		try {
			...
			FileInputFormat.addInputPath(job, new Path("/root/input/friend"));
			
			Path outpath =new Path("/root/output/f1");
